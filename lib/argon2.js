...
(function(root, factory) {
    ...
    async function loadWasmBinary () {
        ...
        if (typeof require === 'function') {
            ...
            const wasmModule = await (simdSupport ? Promise.resolve(require('../dist/argon2-simd.wasm')) : Promise.resolve(require('../dist/argon2.wasm')));
            return decodeWasmBinary(wasmModule);
        }
        // console.log('loading wasm binary - from node_module');
        // no-require
    ...
    /**
     * Argon2 hash
     * @param {Uint8Array} params.pass - password Uint8Array
     * @param {Uint8Array} params.salt - salt Uint8Array
     * @param {number} [params.time=1] - the number of iterations
     * @param {number} [params.mem=8] - used memory, in KiB
     ...
     *
     * @example
     *  argon2.hash({ pass: new Uint8Array(32), salt: new Uint8Array(8) })
     *      .then(h => console.log(h.hash, h.hashHex, h.encoded))
     *      .catch(e => console.error(e.message, e.code))
     */
    function argon2Hash (params) {
        const mCost = params.mem || 8;
        const batchSize = params.batchSize || 32;

        return loadModule(mCost * (params.pass.length / batchSize)).then((Module) => {
            const progressObserver = params.progressObserver;
            const tCost = params.time || 1;
            // console.log(`alloc ${params.pass.length}`)
            const pwd = allocateArray(Module, params.pass);
            // console.log(`alloc ${params.pass.length} done`)
            ...
            const num = Math.floor(params.pass.length / batchSize);
            // console.log(`alloc ${hashlen * num}`)
            const hash = Module.allocate(
                new Array(hashlen * num),
                'i8',
                Module.ALLOC_NORMAL
            );
            // console.log(`alloc ${hashlen * num} done`)
            const secret = null;
            const secretlen = 0;
            const ad = null;
            const adlen = 0;
            ...
            const version = 0x13;
            // console.log(`alloc ${hashlen * num}`);
            const result = new Uint8Array(hashlen * num);
            // console.log(`alloc ${hashlen * num} done`);
            // console.log(hashlen, pwd.length)
            const encodedResult = [];
            let error;
            let res, err;
            try {
            ...
            if (res === 0 && !err) {
                // console.log(`setting ${hashlen * num}`)
                result.set(Module.HEAP8.subarray(hash, hash + hashlen * num));
                // console.log(`set ${hashlen * num}`)
                if (progressObserver) {
                    progressObserver(num, num);
                }
            } else {
                try {
                    err = err || Module.UTF8ToString(Module._argon2_error_message(res));
                } catch (e) {}
