    ...
    function argon2Hash (params) {
        ...
        return loadModule(mCost).then((Module) => {
            ...
            if (batchSize) {
                pwdlen = batchSize;
                pwd = [];
                // console.log(params.pass.length, batchSize)
                for (let i = 0; i < params.pass.length; i += batchSize) {
                    pwd.push(allocateArrayStr(Module, params.pass.subarray(i, i + batchSize)));
                }
                // console.log(pwd)
            } else {
                pwd = [allocateArrayStr(Module, params.pass)];
                pwdlen = pwd[0].length;
            }
            const salt = allocateArrayStr(Module, params.salt);
            ...
            const encodedlen = params.encode ? Module._argon2_encodedlen(
                tCost,
                mCost,
                parallelism,
                saltlen,
            ...
            const encoded = params.encode && Module.allocate(
                new Array(encodedlen + 1),
                'i8',
                Module.ALLOC_NORMAL
            );
            const version = 0x13;
            const result = new Uint8Array(hashlen * pwd.length);
            ...
            for (let i = 0; i < pwd.length; i++) {
                ...
                if (res === 0 && !err) {
                    ...
                    // }
                    result.set(Module.HEAP8.subarray(hash, hash + hashlen), i * hashlen);
                    if (params.encode) {
                        encodedResult.push(Module.UTF8ToString(encoded));
                    }
                    if (progressObserver) {
                        progressObserver(i, pwd.length);
                    }
                } else {
                    try {
            ...
            try {
                ...
                Module._free(salt);
                Module._free(hash);
                params.encode && Module._free(encoded);
                if (ad) {
                    Module._free(ad);
                }
                if (secret) {
